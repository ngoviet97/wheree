...
column_a_data = [row for row in column_a_data if (row['uuid'], 'main', 'banner') not in existing_urls]
...
for row in column_a_data:
    ...
    if len(imgs) == 0:
        headerImg = [row['uuid'], '','main','banner']
    else:
        headerImg = [row['uuid'], imgs[0],'main','banner']
